...
data(galNRC4_01,package="CustomMicroarrayPackage")
gridNameTable = galNRC4_01
...
chipDataL <- readAndProcessChipData( currDir=gprDir,GALFile = gridNameTable )
RG = chipDataL$RG
RG$genes$ID = ifelse( RG$genes$ID == "","noSig",RG$genes$ID )
RG$genes$Name = ifelse( RG$genes$Name == "","noSig",RG$genes$Name )
RG$genes = addProbeTypeAndChipTypeToRGGenes(RG=RG,dbcon=db ) # processing the chip
message( paste0("All Oligos are present on the chip: ", dim(RG$genes[-c( grep("unknown",RG$genes$ID),grep("noSig",RG$genes$ID)),])[1]/4 == 3977 ) )
##########################################################
...
ndups = 4 #VARDEFINITION
spacing = 1  #VARDEFINITION
minDetectLim = 0.15  #VARDEFINITION (minimal detection limit)
subsetByManualControl = TRUE  #VARDEFINITION (#)filtering option)
...
````{r, echo=TRUE}
# analysisTemplate2.0 = readLines("/media/Rstick/ADpaper/workspace/CustomMicroarrayPackage/src/AnalysisTemplate2_0.Rmd")
# save(analysisTemplate2.0, file="/media/Rstick/ADpaper/workspace/CustomMicroarrayPackage/data/analysisTemplate2.0.rda")
```
